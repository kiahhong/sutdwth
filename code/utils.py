...
def prepare_data_ptb_xl(path_to_ptb_xl, path_to_out_folder, min_cnt=1):
    
    # reading df
    ptb_xl_csv = os.path.join(path_to_ptb_xl, "ptbxl_database.csv")
    df_ptb_xl=pd.read_csv(ptb_xl_csv, index_col="ecg_id")
    
    # --- START OF MODIFICATION ---
    # This is the new part that processes and saves the .npy files
    print("Processing raw ECG signals and saving as .npy files...")
    for index, row in tqdm(df_ptb_xl.iterrows(), total=df_ptb_xl.shape[0]):
        # Construct path to the raw .dat file
        raw_signal_path = os.path.join(path_to_ptb_xl, row.filename_hr)
        
        # Load the raw signal data using wfdb
        signal, meta = wfdb.rdsamp(raw_signal_path)
        original_fs = meta['fs']
        
        # Transpose signal to (leads, length) and preprocess
        processed_signal = ecg_preprocessing(signal.T, original_fs)
        
        # Construct the output filename and path
        output_filename = "HR" + os.path.basename(row.filename_hr).replace("_hr", ".hea.npy")
        output_path = os.path.join(path_to_out_folder, output_filename)
        
        # Save the processed signal as a .npy file
        np.save(output_path, processed_signal)
    print("Finished processing and saving .npy files.")
    # --- END OF MODIFICATION ---
    ...
    # preparing labels
    ptb_xl_label_df = pd.read_csv(os.path.join(path_to_ptb_xl, "scp_statements.csv"))
    ...
    ptb_all.to_csv(os.path.join(path_to_out_folder, "ptb_all.csv"), index=False)
    ptb_form.to_csv(os.path.join(path_to_out_folder, "ptb_form.csv"), index=False)
    ptb_rhythm.to_csv(os.path.join(path_to_out_folder, "ptb_rhythm.csv"), index=False)
    ptb_diag.to_csv(os.path.join(path_to_out_folder, "ptb_diag.csv"), index=False)
    ptb_diag_subclass.to_csv(os.path.join(path_to_out_folder, "ptb_diag_subclass.csv"), index=False)
    ptb_diag_superclass.to_csv(os.path.join(path_to_out_folder, "ptb_diag_superclass.csv"), index=False)
