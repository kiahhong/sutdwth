...
def finetune(args):
    ...
    if torch.backends.mps.is_available():
        ...
        logger.info("Using NVIDIA CUDA device.")
    else:
    ...
    # Construct the absolute path to the processed_data directory
    absolute_ecg_dir_path = os.path.join(script_dir, "..", "processed_data")
    # --- END OF MODIFICATION ---

    train_set = ECGDataset(
        path_to_dataset_csv=args.path_to_dataset_csv_train,
        ecg_dir_path=absolute_ecg_dir_path,
    ...
    if args.resume_finetuning:
        ...
        # Add the missing attribute to the loaded config if it doesn't exist
        ...
        for ecg, attention_mask, labels in tqdm(train_dl, total=len(train_dl)):
            ...
            if global_step % args.val_interval == 0:
                ...
                if val_loss <= best_val_loss - MINIMAL_IMPROVEMENT: 
                    ...
                    save_dir = os.path.join(SUPERVISED_MODEL_CKPT_PATH, args.sweep_dir)
...
if __name__ == "__main__":
    ...
    # This check is now handled by the device selection logic at the start of the finetune function.
